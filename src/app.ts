...
app.use(function errorHandler(err: unknown, req: ExRequest, res: ExResponse, next: NextFunction): ExResponse | void {
   ...
   if (err instanceof ValidateError) {
      ...
      return res.status(422).json(new ApiResponse<any>(undefined, 'VALIDATION_ERROR', undefined, err ? err.fields : null));
   }
   if (err instanceof Error) {
      if ((err as any).status) {
         return res.status((err as any).status).json(new ApiResponse<any>(undefined, 'ERROR', err.message));
      }
      return res.status(500).json(new ApiResponse<any>(undefined, 'ERROR', err.message));
   }
   next();
});
