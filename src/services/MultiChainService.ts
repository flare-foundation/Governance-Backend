...
export class MultiChainService {
  ...
  async init() {
    ...
    for (let config of this.chainConfigs) {
      ...
      this.addressToContactInfo[config.CHAIN_ID] =
        this.addressToContactInfo[config.CHAIN_ID] || {};
      for (let deployed of JSON.parse(
        fs.readFileSync(deployFname).toString()
      ) as ContractDeploy[]) {
      ...
      for (let contractDeploy of this.deployData) {
        ...
        let { contract, abi } = await getWeb3ContractWithAbi(
          this.web3[config.CHAIN_ID],
        ...
        this.addressToContactInfo[config.CHAIN_ID][
          contractDeploy.address.toLowerCase()
        ] = contractDeploy;
  ...
  public async getContractFromAddress(
    chainId: number,
    address: string
  ): Promise<PollingAccept | PollingReject> {
    await this.waitForInitialization();
    let deployInfo = this.addressToContactInfo[chainId]?.[address.toLowerCase()];
    if (deployInfo) {
      return this.deployMap[chainId]?.[deployInfo.name] as
        | PollingAccept
        | PollingReject;
    }
  }
  ...
  public async governanceVotePower(
    chainId: number
  ): Promise<GovernanceVotePower> {
    return (await this.getContract(
      chainId,
      "GovernanceVotePower"
    )) as GovernanceVotePower;
  }
  ...
  public async votePowerForProposalId(
    chainId: number,
    voterAddress: string,
    votePowerBlock: number
  ): Promise<string> {
    const votePowerC = await this.governanceVotePower(chainId);
    return await votePowerC.methods
      .votePowerOfAt(voterAddress, votePowerBlock)
      .call();
  }
}
